...
static time_t g_vert_mtime = 0;
static time_t g_frag_mtime = 0;
 ...
 * Graphics pipeline
 * -------------------------------------------------------------------------- */
/* Load a SPIR-V file from disk. Returns NULL on failure. Caller must free(). */
static uint32_t* load_spv_file(const char* path, size_t* out_size) {
...
static void create_pipeline(void) {
    ...
    if (ql_vert) free(ql_vert);
    if (ql_frag) free(ql_frag);
...
static int check_shader_reload(void) {
    ...
    if (g_vert_mtime == 0) g_vert_mtime = vert_t;
    if (g_frag_mtime == 0) g_frag_mtime = frag_t;
    return 0;
}
...
static void reload_pipeline(void) {
    ...
    create_pipeline();
    printf("[HOT RELOAD] Pipeline rebuilt successfully!\n");
}
...
int main(void) {
    ...
    while (g_running) {
        ...
        if (g_running) {
            /* Check for shader hot reload every N frames */
            g_frame_counter++;
            if (g_frame_counter >= HOT_RELOAD_CHECK_INTERVAL) {
                g_frame_counter = 0;
