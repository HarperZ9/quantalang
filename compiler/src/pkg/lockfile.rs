...
impl Lockfile {
    ...
    pub fn to_string(&self) -> String {
        ...
        // Header
        writeln!(output, "# This file is automatically generated by quanta-pkg.").unwrap();
        writeln!(output, "# Do not edit manually.").unwrap();
        writeln!(output).unwrap();

        // Lockfile section
        writeln!(output, "[lockfile]").unwrap();
        writeln!(output, "version = {}", self.version).unwrap();
        writeln!(output, "root = \"{}\"", self.root).unwrap();
        writeln!(output).unwrap();
        ...
        for (name, pkg) in &self.packages {
            ...
            writeln!(output, "source = \"{}\"", pkg.source).unwrap();

            if let Some(checksum) = &pkg.checksum {
                writeln!(output, "checksum = \"{}\"", checksum).unwrap();
            }
            ...
            if !pkg.dependencies.is_empty() {
                ...
                for (dep_name, dep_ver) in &pkg.dependencies {
                    writeln!(output, "{} = \"{}\"", dep_name, dep_ver).unwrap();
                }
            }
            ...
            if !pkg.features.is_empty() {
                ...
                for feature in &pkg.features {
                    writeln!(output, "{} = true", feature).unwrap();
                }
            }
...
mod tests {
    ...
    #[test]
    fn test_package_source_parse() {
        let registry = PackageSource::parse("registry+https://registry.quantalang.org").unwrap();
        assert!(matches!(registry, PackageSource::Registry { .. }));

        let git = PackageSource::parse("git+https://github.com/user/repo#abc123").unwrap();
        assert!(matches!(git, PackageSource::Git { rev, .. } if rev == "abc123"));

        let path = PackageSource::parse("path+../local-pkg").unwrap();
    ...
    fn test_lockfile_diff() {
        ...
        new.packages.get_mut("b").unwrap().version = Version::new(2, 0, 0);
