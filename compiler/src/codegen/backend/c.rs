...
impl CBackend {
    ...
    fn generate_type_definitions(&mut self, types: &[MirTypeDef]) -> CodegenResult<()> {
        ...
        for ty in types {
            ...
            match &ty.kind {
                ...
                TypeDefKind::Enum { discriminant_ty: _, variants } => {
    ...
    fn generate_vtable_types(&mut self, module: &MirModule) -> CodegenResult<()> {
        ...
        self.output.push_str("// Vtable types for dynamic dispatch\n");
    ...
    fn generate_vtable_instances(&mut self, module: &MirModule) -> CodegenResult<()> {
        ...
        for vtable in &module.vtables {
            for (method_name, mangled_fn, sig) in &vtable.methods {
                let ret = self.type_to_c(&sig.ret);
                let wrapper_name = format!("__vtable_wrap_{}_{}_{}",
                ...
                write!(self.output, "static {} {}({}) {{ return {}({}); }}\n",
                    ret, wrapper_name,
                    wrapper_params.join(", "),
                    mangled_fn,
                    call_args.join(", ")
        ...
        for vtable in &module.vtables {
            write!(self.output, "static {}_vtable {}_{}_vtable_instance = {{\n",
                vtable.trait_name, vtable.type_name, vtable.trait_name).unwrap();
            ...
            for (method_name, _, sig) in &vtable.methods {
                ...
                let wrapper_name = format!("__vtable_wrap_{}_{}_{}",
                    vtable.type_name, vtable.trait_name, method_name);
                write!(self.output, "    .{} = ({} (*)({})){},\n",
                    method_name, ret, params.join(", "), wrapper_name
                ).unwrap();
            }
            write!(self.output, "}};\n\n").unwrap();
        }
    ...
    fn generate_string_table(&mut self, strings: &[Arc<str>]) -> CodegenResult<()> {
        ...
        for (i, s) in strings.iter().enumerate() {
            ...
            write!(self.output, "static const char* __str{} = \"{}\";\n", i, escaped).unwrap();
        }
        self.output.push('\n');
    ...
    fn generate_forward_declarations(&mut self, functions: &[MirFunction]) -> CodegenResult<()> {
        ...
        let extern_fns: Vec<_> = functions.iter()
            .filter(|f| f.is_declaration() && f.sig.calling_conv == CallingConv::C)
            .collect();
        let regular_fns: Vec<_> = functions.iter()
            .filter(|f| !(f.is_declaration() && f.sig.calling_conv == CallingConv::C))
            .collect();
        ...
        if !extern_fns.is_empty() {
            let std_c_fns = [
                ...
                "puts", "putchar", "getchar", "gets", "fgets", "fputs",
        ...
        if params.is_empty() {
            ...
            for (i, param) in params.iter().enumerate() {
                ...
                let name = param.name.as_ref()
                    .map(|n| {
                        let s = n.to_string();
                        if Self::is_c_reserved(&s) { format!("_{}", s) } else { s }
                    })
                ...
                if matches!(param.ty, MirType::Array(_, _)) {
                    ...
                    let ret = self.type_to_c(&sig.ret);
                    let fn_params: Vec<_> = sig.params.iter()
                        .map(|p| self.type_to_c(p))
                        .collect();
                    write!(self.output, "{} (*{})({})",
                        ret,
    ...
    fn generate_statement(&mut self, stmt: &MirStmt, locals: &[MirLocal]) -> CodegenResult<()> {
        match &stmt.kind {
            MirStmtKind::Assign { dest, value } => {
                ...
                let is_array_dest = locals.get(dest.0 as usize)
                ...
                if is_array_dest {
                    if let MirRValue::Aggregate { operands, .. } = value {
                        ...
                        write!(self.output, "memcpy({}, {}, sizeof({}));\n",
                            dest_name, src, dest_name).unwrap();
                    } else {
                        self.write_indent();
                        let rvalue = self.rvalue_to_c(value, locals)?;
                        write!(self.output, "memcpy({}, &({}), sizeof({}));\n",
                            dest_name, rvalue, dest_name).unwrap();
                    }
                } else if let MirRValue::FieldAccess { base, field_name, .. } = value {
                    ...
                    if field_name.as_ref() == "handler_data" {
                        ...
                        let dest_type = locals.get(dest.0 as usize)
                            .map(|l| self.type_to_c(&l.ty))
                            .unwrap_or_else(|| "int32_t".to_string());
                        self.write_indent();
                        write!(self.output, "{} = *({}*){}.handler_data;\n",
                            dest_name, dest_type, base_str).unwrap();
                    } else {
                        self.write_indent();
                        let rvalue = self.rvalue_to_c(value, locals)?;
            ...
            MirStmtKind::FieldDerefAssign { ptr, field_name, value } => {
                let ptr_name = self.local_name(*ptr, locals);
                self.write_indent();
                let rvalue = self.rvalue_to_c(value, locals)?;
    ...
    fn generate_terminator(&mut self, term: &MirTerminator, locals: &[MirLocal], blocks: &[MirBlock]) -> CodegenResult<()> {
        match term {
            ...
            MirTerminator::If { cond, then_block, else_block } => {
                ...
                write!(self.output, "if ({}) goto {}; else goto {};\n",
                    cond_str, self.block_label(then_block, blocks), self.block_label(else_block, blocks)
                ).unwrap();
            }
            MirTerminator::Switch { value, targets, default } => {
                ...
                for (const_val, target) in targets {
                    self.write_indent();
                    write!(self.output, "case {}: goto {};\n",
                        self.const_to_c(const_val),
                        self.block_label(target, blocks)
                    ).unwrap();
                }
                self.write_indent();
                write!(self.output, "default: goto {};\n", self.block_label(default, blocks)).unwrap();
                self.indent -= 1;
                self.writeln("}");
            }
            MirTerminator::Call { func, args, dest, target, .. } => {
                ...
                if func_str.starts_with("__vtable_dispatch_") {
                    ...
                    if parts.len() == 2 {
                        ...
                        let method_name = trait_and_method.rsplit('_').next().unwrap_or(trait_and_method);

                        let args_str: Vec<_> = args.iter()
                            .map(|a| self.value_to_c(a, locals))
                            .collect();
                        ...
                        if let Some(dest_local) = dest {
                            ...
                            write!(self.output, "{} = ((void*){} != NULL) ? ",
                                dest_name, vtable_local_name).unwrap();
                            // Cast vtable to correct type and call through function pointer
                            let trait_name = trait_and_method.rsplitn(2, '_').last().unwrap_or("Unknown");
                            write!(self.output, "(({}_vtable*){})->{}({}) : 0;\n",
                                trait_name, vtable_local_name, method_name,
                                args_str.join(", ")
                            ).unwrap();
                        }
                ...
                if let Some(target_block) = target {
                    self.write_indent();
                    write!(self.output, "goto {};\n", self.block_label(target_block, blocks)).unwrap();
                }
            }
            ...
            MirTerminator::Drop { place: _, target, .. } => {
    ...
    fn rvalue_to_c(&self, rvalue: &MirRValue, locals: &[MirLocal]) -> CodegenResult<String> {
        Ok(match rvalue {
            ...
            MirRValue::BinaryOp { op, left, right } => {
                ...
                if *op == BinOp::Pow { return Ok(format!("pow({}, {})", l, r)); }
                ...
                match kind {
                    ...
                    AggregateKind::Variant(name, disc, variant_name) => {
                        if vals.is_empty() {
                            ...
                            format!("({}){{ .tag = {}, .data = {{ .{} = {{ {} }} }} }}",
                                name, disc, variant_name, vals.join(", "))
                        }
                    }
            ...
            MirRValue::VariantField { base, variant_name, field_index, .. } => {
                let base_str = self.value_to_c(base, locals);
        ...
        locals.get(id.0 as usize)
            ...
            .map(|n| {
                ...
                let has_dup = locals.iter().any(|other| {
                    other.id != id
                        && other.name.as_ref().map(|s| s.as_ref()) == Some(n.as_ref())
                });
                if has_dup {
                    format!("{}_{}", base, id.0)
                } else {
...
mod tests {
    ...
    fn test_c_backend_struct_aggregate() {
        ...
        builder.aggregate(p, AggregateKind::Struct(Arc::from("Point")),
        ...
        let code = output.as_string().unwrap();
        assert!(code.contains("(Point){ 3, 4 }"), "Expected struct aggregate in:\n{}", code);
    }
    ...
    fn test_c_backend_enum_type() {
        ...
        let code = output.as_string().unwrap();
        assert!(code.contains("Shape_Circle = 0"), "Expected enum tag in:\n{}", code);
        assert!(code.contains("Shape_Rectangle = 1"), "Expected enum tag in:\n{}", code);
        assert!(code.contains("Shape_Tag"), "Expected tag type in:\n{}", code);
        assert!(code.contains("} Shape;"), "Expected struct typedef in:\n{}", code);
    }
    ...
    fn test_c_backend_variant_field_access() {
        ...
        builder.assign(result, MirRValue::VariantField {
            base: values::local(LocalId(0)),
        ...
        let code = output.as_string().unwrap();
        assert!(code.contains("s.data.Circle.f0"), "Expected variant field access in:\n{}", code);
    }
...
"#;
        ...
        assert!(code.contains("typedef struct Point"), "Missing struct typedef in:\n{}", code);
        assert!(code.contains("int32_t x;"), "Missing field x in:\n{}", code);
        assert!(code.contains("int32_t y;"), "Missing field y in:\n{}", code);
        assert!(code.contains("} Point;"), "Missing struct close in:\n{}", code);

        // Verify struct literal
        assert!(code.contains("(Point){ 3, 4 }"), "Missing struct literal in:\n{}", code);

        // Verify field access
        assert!(code.contains("p.x"), "Missing field access in:\n{}", code);

        // Verify function signature uses struct type
        assert!(code.contains("Point p"), "Missing struct param in:\n{}", code);
    }
...
"#;
        ...
        assert!(code.contains("Shape_Circle = 0"), "Missing Circle tag in:\n{}", code);
        assert!(code.contains("Shape_Rect = 1"), "Missing Rect tag in:\n{}", code);
        assert!(code.contains("Shape_Tag"), "Missing tag type in:\n{}", code);
        assert!(code.contains("} Shape;"), "Missing enum close in:\n{}", code);
        assert!(code.contains("union"), "Missing union in:\n{}", code);

        // Verify variant field access in match
        assert!(code.contains(".data.Circle.f0"), "Missing variant field access in:\n{}", code);
        assert!(code.contains(".data.Rect.f0"), "Missing Rect field0 access in:\n{}", code);
        assert!(code.contains(".data.Rect.f1"), "Missing Rect field1 access in:\n{}", code);
