// ===============================================================================
// BUILDLANG CODE GENERATOR - HLSL BACKEND
// ===============================================================================
...
impl HlslBackend {
    ...
    pub fn generate_reshade(&mut self, module: &MirModule) -> CodegenResult<String> {
        ...
        self.output
            .push_str("// Do not edit generated functions - regenerate from .bld source\n\n");
        ...
        if let Some(frag) = fragment_fn {
            ...
            self.writeln(&format!("technique Build_{} {{", frag.name));
            self.indent += 1;
            self.writeln("pass {");
            self.indent += 1;
            ...
            self.output
                .push_str("    // TODO: Apply your BuildLang functions to 'color' here\n");
            self.output.push_str("    return float4(color, 1.0);\n");
            self.output.push_str("}\n\n");
            self.output.push_str("technique BuildShader {\n");
    ...
    fn generate_struct(&mut self, ty: &MirTypeDef) -> CodegenResult<()> {
        ...
        match ty.name.as_ref() {
            "build_vec2" | "build_vec3" | "build_vec4" | "build_mat4" => return Ok(()),
            _ => {}
        }
        match &ty.kind {
    ...
    fn generate_function(&mut self, func: &MirFunction) -> CodegenResult<()> {
        ...
        if matches!(func.shader_stage, Some(ShaderStage::Fragment)) {
            let has_pos = func
                ...
                .params
                .iter()
                .any(|ty| matches!(ty, MirType::Struct(n) if n.as_ref() == "build_vec4"));
            if !has_pos {
            ...
            MirRValue::Aggregate { kind, operands } => {
                match kind {
                    AggregateKind::Struct(name) => {
                        ...
                        // Map build_vec types to HLSL constructors
                        match name.as_ref() {
                            "build_vec2" => format!("float2({})", fields.join(", ")),
                            "build_vec3" => format!("float3({})", fields.join(", ")),
                            "build_vec4" => format!("float4({})", fields.join(", ")),
        ...
        match value {
            ...
            MirValue::Global(name) | MirValue::Function(name) => {
                ...
                match name.as_ref() {
                    ...
                    "fabs" => "abs".to_string(),
                    // Vector math
                    "build_dot2" | "build_dot3" | "build_dot4" => "dot".to_string(),
                    "build_normalize2" | "build_normalize3" | "build_normalize4" => {
                        "normalize".to_string()
                    }
                    "build_length2" | "build_length3" | "build_length4" => "length".to_string(),
                    "build_cross" => "cross".to_string(),
                    "build_reflect3" => "reflect".to_string(),
                    // Interpolation
                    "build_mix" | "build_lerp2" | "build_lerp3" | "build_lerp4" => {
                        "lerp".to_string()
                    }
                    // Shader math
                    "build_clampf" | "build_clamp3" => "clamp".to_string(),
                    "build_smoothstep" => "smoothstep".to_string(),
                    "build_fract" => "frac".to_string(),
                    "build_step" => "step".to_string(),
                    // Min/max
                    "build_min_f64" | "build_min_f32" | "build_min_i32" | "build_min_i64" => {
                        "min".to_string()
                    }
                    "build_max_f64" | "build_max_f32" | "build_max_i32" | "build_max_i64" => {
                        "max".to_string()
                    }
                    // Texture sampling
                    "texture_sample" | "build_texture_sample" => "tex2D".to_string(),
                    // Vector constructors (mapped by aggregate, but handle if called as function)
                    "build_vec2_new" => "float2".to_string(),
                    "build_vec3_new" => "float3".to_string(),
                    "build_vec4_new" => "float4".to_string(),
                    // Matrix
                    "build_mat4_mul" | "build_mat4_mul_vec4" => "mul".to_string(),
                    // Strip build_ prefix for functions that map directly
                    _ => name.to_string(),
                }
            }
        }
    ...
    fn type_to_hlsl(&self, ty: &MirType) -> String {
        match ty {
            ...
            MirType::Struct(name) => {
                // Map BuildLang built-in types to HLSL types
                match name.as_ref() {
                    "build_vec2" => "float2".to_string(),
                    "build_vec3" => "float3".to_string(),
                    "build_vec4" => "float4".to_string(),
                    "build_mat4" => "float4x4".to_string(),
                    "BuildString" => "string".to_string(), // Not really valid in HLSL
                    _ => name.to_string(),
                }
            }
